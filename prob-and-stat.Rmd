...
To complete the test, we need to obtain the p-value of the test. Since this is a one-sided test with a less-than alternative, we need to compute the area to the left of `r t` for a $t$ distribution with `r n - 1` degrees of freedom. That is,
...
Suppose we would like to calculate $P(0 < D < 2)$. First we will 
